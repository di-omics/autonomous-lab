...
def test_the_proposer_cannot_change_the_answer():
  ...
  step = SINGLE_CELL_GENOMICS.steps[7]  # the STAR PTA/WGA leg
...
def test_a_failing_gate_refuses_a_step_that_is_otherwise_runnable():
  ...
  impossible = gate_for("sequencing_controls")  # UNMEASURABLE on any workcell here
...
def test_every_blocker_is_reported_not_just_the_first():
  ...
  d = decide(Request(step), _wired(), gates=[gate_for("sequencing_controls")])
  assert any("capability" in r for r in d.reasons)
  assert any("sequencing_controls" in r for r in d.reasons)


def test_a_supervised_step_is_granted_supervised_not_granted():
  step = SINGLE_CELL_GENOMICS.steps[7]  # STAR pta_wga_lysis, validated run card
...
def test_an_unmeasurable_gate_distinguishes_a_broken_instrument_from_a_missing_input():
  ...
  step = _zero_decode_step()
  broken = decide(Request(step), _wired(), gates=[gate_for("library_loading_window")])
  ...
  missing = decide(Request(step), _wired(), gates=[gate_for("thermal_headroom")])
