...
def test_declared_but_unmerged_instrument_is_not_a_crash():
  ...
  from autonomous_lab.registry import declared
...
def test_written_is_distinct_from_manual_and_supervised():
  ...
  got = {
    ...
    for op in ("library_pool", "pcr_enrichment_round1_cleanup", "pta_wga_lysis")
  }
  assert got == {
...
def test_a_user_protocol_that_declares_its_artifacts_gets_its_hops_counted():
  from autonomous_lab.model import Artifact
...
def test_split_endpoint_parses_and_refuses_ambiguity():
  from autonomous_lab.executor import _split_endpoint
