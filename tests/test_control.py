...
def _bounds():
  return [
    Bound("setting_a", 0.0, 1.0, "fraction", "synthetic test range"),
    Bound("setting_b", 10.0, 20.0, "arbitrary units", "synthetic test range"),
  ]
...
def _obs(n, tier=EvidenceTier.MEASURED):
  ...
  for i in range(n):
    out.append(
      Observation(
        params={"setting_a": i / max(1, n), "setting_b": 10.0 + i},
        metrics={"response": 2.0 * i},
        tier=tier,
      )
    )
  return out
...
def test_ranking_is_by_expected_waste_not_by_failure_rate():
  ...
  for _ in range(20):
    m.observe("pcr_enrichment_round1_cleanup", False)  # step 10, fails always
  ranked = m.rank_by_expected_waste(SINGLE_CELL_GENOMICS)
  assert [w.step for w in ranked][0] == "pcr_enrichment_round1_cleanup"
...
def test_the_controller_cannot_move_a_criterion():
  ...
  is to delete it."""
  c = Criterion("response", ">=", 5.0, "x", "synthetic test rule", origin=Origin.TRANSCRIBED)
  ctrl = Controller(_bounds(), "response", criteria=[c])
...
def test_the_controller_refuses_rather_than_proposing_a_run_it_expects_to_fail():
  impossible = Criterion(
    "response", ">=", 1e9, "x", "a synthetic threshold nothing can reach",
    origin=Origin.TRANSCRIBED,
  )
  ctrl = Controller(_bounds(), "response", criteria=[impossible])
...
def test_no_proposal_below_the_minimum_number_of_real_observations():
  result = Controller(_bounds(), "response").propose(_obs(2))
  assert isinstance(result, Refusal)
...
def test_modeled_and_simulated_observations_do_not_train():
  ...
  for tier in (EvidenceTier.MODELED, EvidenceTier.SIMULATED):
    result = Controller(_bounds(), "response").propose(_obs(20, tier=tier))
    assert isinstance(result, Refusal), tier
    assert result.observations == 0


def test_measured_observations_do_train():
  result = Controller(_bounds(), "response").propose(_obs(8))
...
def test_a_proposal_carries_a_calibrated_interval_once_there_is_enough_data():
  result = Controller(_bounds(), "response").propose(_obs(10))
...
def test_an_observations_tier_can_be_earned_instead_of_declared():
  ...
  good = Measurement(
    "setpoint_error", 0.4, "profile_units", "odtc", "pcr_enrichment_round1",
    claimed=EvidenceTier.MEASURED,
  )
  # Claims just as loudly, off an instrument whose run card has never returned a number.
  bad = Measurement(
    "loading_concentration", 0.5, "profile_units", "tecan", "read_absorbance",
    claimed=EvidenceTier.MEASURED,
  )
...
def test_a_bound_without_a_source_is_refused():
  with pytest.raises(ValueError, match="no source"):
    Bound("setting_a", 0.0, 1.0, "fraction", "")


def test_an_empty_bound_is_refused():
  with pytest.raises(ValueError, match="empty"):
    Bound("setting_a", 1.0, 0.0, "fraction", "backwards")


def test_a_parameter_the_surrogate_has_no_bound_for_raises():
  ctrl = Controller(_bounds(), "response")
  with pytest.raises(KeyError, match="missing"):
    ctrl.surrogate._encode({"setting_a": 0.5})
