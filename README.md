...
autonomous-lab schedule --plr-tested ../plr-tested single_cell_genomics --plates 10  # workcell serialization
...
real hardware under supervision. Proving run cards therefore moves stalls from `decode` to
`trust` rather than removing them, and the run stops in the same place for a different
reason.
...
156 device-free tests. The ones that matter most try to make the layer lie:
