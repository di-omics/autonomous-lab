...
$ autonomous-lab doctor --plr-tested ../plr-tested
  [ok  ] star.pta_wga_lysis  run card exists: liquid-handler/starlab_live/00_pta_wga_1col_src1lysis_src3rxn_dst1_hhs_DRY.py
  [ok  ] star.pta_wga_lysis  confirm token appears in the run card: RUN_SINGLE_COL_WGS_PREP_HHS
  ...
...
when the whole-genome preparation run card actually gates on `RUN_SINGLE_COL_WGS_PREP_HHS`. The ledger was
...
1. **Let an instrument's reputation transfer to a step.** plr-tested has a validated
   whole-genome preparation addition and a validated targeted PCR choreography; it has no validated bead cleanup
...
- `single_cell_genomics` -- Namocell sort, STAR whole-genome preparation, ODTC targeted PCR round 1, STAR library
