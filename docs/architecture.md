...
Clair is a laboratory intelligence kernel, not an all-powerful robot agent. It keeps a
...
Clair currently implements the scientific/evidence spine and a deliberately narrow
...
Clair/plr-re source identities, and content hashes for every external federated run-card
...
flowchart LR
  ...
  Source -->|aliquot + library prep| A
  Source -->|aliquot + library prep| B
...
such as PCR must instead be labeled as a transformation with a scientific reason; its
input allocation is still conserved and auditable even when output quantity is not.
...
Clair's human-centered division of labor is intentional.
...
- independent wet-lab validation of the integrated Clair loop.
