# First pilot: one bounded laboratory workflow

The first deployment should be one scientifically meaningful workflow in one external
lab that reduces coordination and
...
A stable, repeatable process with explicit measurements and acceptance rules is the best
starting point because it exposes both supported and unsupported boundaries.
...
Run the process unchanged while the evidence layer observes:
...
- hands-on minutes per passing output;
...
Patterns that repeat across integrations become reusable capabilities:
...
The durable asset is the verified mapping between scientific intent, physical execution,
observed evidence, and recovery.
