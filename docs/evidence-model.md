...
The evidence layer makes the origin and strength of a claim first-class. The same numeric value means
...
comparison with a signed or externally anchored expected head from an earlier checkpoint. The chain also
...
constraint ID, `boolean` unit, a `0` or `1` value, and a matching boolean label. The
learning layer validates those fields, their evidence, and their freshness before an observation is
