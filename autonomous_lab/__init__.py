...
four layers that each refuse to overstate what the one below them supports.
  ...
  UNMEASURABLE verdict for the case a gate in front of a broken instrument must not answer.
      ...
      criteria.get("library_loading_window").evaluate(measurements, workcell)
...
would quietly omit, and three of the five reference gates evaluate to UNMEASURABLE because
the plate reader they depend on has never returned a plate.
"""
