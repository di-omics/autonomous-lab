...
Four ideas, each of which exists to stop a gate from passing something it should not.
  ...
  empty `source`. Thresholds are where invented numbers do their damage: a made-up 0.9x
  bead ratio or a guessed minimum yield looks exactly like a real one in code, and it will
  be obeyed. Requiring provenance on the number makes an unsourced threshold a
  construction error rather than a silent decision. Put the protocol section, the vendor
  spec, or the internal validation run in the field. "TODO: confirm with the assay owner"
...
class Criterion:
  ...
  def describe(self) -> str:
    return f"{self.metric} {self.comparator} {self.threshold:g} {self.units}"
...
class ConformalBand:
  ...
  noisy regime is the low-input regime, and low input is the whole assay.
...
class Gate:
  ...
  def ready_for_hardware(self) -> Tuple[bool, List[str]]:
    ...
    """
    reasons = [
      f"{c.metric}: threshold {c.threshold:g} {c.units} is {c.origin.value} ({c.source})"
  ...
  def evaluate(
    ...
    for c in self.criteria:
      ...
      # and the person who can fix it is the assay owner rather than the operator.
