...
claiming more than it can support.
  ...
  into one tube and a downstream measurement refers only to the combined pool. If each
...
def reference_lineage(n_cells: int = 96, indexed: bool = False) -> Lineage:
  ...
  for w in addrs:
    lin.derive(f"cell_{w}", f"lysate_{w}", "wgs_prep_lysis", "star")
    lin.derive(f"lysate_{w}", f"amp_{w}", "pcr_enrichment_round1", "odtc")
    lin.derive(f"amp_{w}", f"lib_{w}", "pcr_enrichment_round1_cleanup", "star")
