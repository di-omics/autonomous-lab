...
def _gate_catalog(args):
  path = getattr(args, "evidence_profile", None)
  if not path:
    return criteria.REFERENCE_GATES
  return criteria.build_reference_gates(criteria.load_profile(path))


def _gates(args) -> int:
  """The acceptance rubrics, and whether this bench can actually apply them."""
  wc = _workcell(args)
  catalog = _gate_catalog(args)
  blocked = 0
  for name in sorted(catalog):
    gate = criteria.get(name, catalog)
  ...
  transcribed = sum(
    1 for g in catalog.values() for c in g.criteria
    if c.origin is acceptance.Origin.TRANSCRIBED
  )
  total = sum(len(g.criteria) for g in catalog.values())
  print(f"\n  {transcribed} of {total} thresholds are transcribed from a citable source.")
  print(f"  {blocked} of {len(catalog)} rubrics cannot gate material yet.")
...
def _session(args) -> int:
  ...
  for step in p.steps:
    sess.request(
      ...
      gates=criteria.gates_for(step.op, catalog),
    )
  print(f"protocol: {p.name}\n{p.summary}\n")
...
def build_parser() -> argparse.ArgumentParser:
  ...
  gt.add_argument(
    "--evidence-profile",
    help="JSON file containing operator-owned thresholds and their cited sources",
  )
  common(gt)
